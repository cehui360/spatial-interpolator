...
warning  off all
%% Input modeling_data
data = dlmread('s53-modeling.txt');
minx = min(data(:,1));
maxx = max(data(:,1));
miny = min(data(:,2));
maxy = max(data(:,2));
%% Input checking_data
testdata = dlmread('s53-checking.txt');
...
Num = size(testdata,1);
%% Parameter setting
para = struct('iterations',1,'nc',5,'lamba',2.5,'d',1.5,'h',2.5,'q',20,'g',1.5);
...
q = para.q; g = para.g;
zs = triple_predictLocal(estimate,para);%estimate the elevations of all DEM grid cells  by using trilateral kernel function.
zs = reshape(zs,r*c,1);
estimate = [xg yg zs];
for i = 1:iterations
    disp(['iteration ', num2str(i)]);
    zs = quadruple_predictLocal(estimate,para);%estimate the elevations of all DEM grid cells  by using quadrilateral  kernel function.
    zs = zs';
    h = h - 0.01;
    d = d - 0.01;
    lamba = lamba + 0.01;
